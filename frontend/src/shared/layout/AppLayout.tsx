...
export function AppLayout() {
	...
	function parseJwtPayload<T = Record<string, unknown>>(
		token: string,
	): T | null {
		try {
		...
		return navItems
			...
			.map((item) => ({
				...
				template: (menuItem) => (
					...
					>
						<i
							className={String(menuItem.icon)}
							aria-hidden="true"
						/>
			...
			<>
				...
				{showLabels && (
					<div className="sidebar-profile">
						...
						<div>
							<strong>
								{user?.perfilNome ?? user?.role ?? "Aluno"}
							</strong>
	...
	return (
		<div
			className={
				sidebarOpen ? "app-shell" : "app-shell app-shell--collapsed"
			}
		>
			...
			<div className="app-main">
				<header className="topbar">
					...
					>
						<Avatar
							label={user ? getInitials(user.nome) : "AL"}
							shape="circle"
						/>
						<div>
